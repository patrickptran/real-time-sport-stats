...
import type {
  ...
  MatchResponse,
  CommentaryResponse,
  CommentaryInsertResult,
  MatchCreatePayload,
  SeedErrorWithContext,
  CricketPeriod,
  TeamDesignation,
} from "./types.js";
...
async function readJsonFile(filePath: string): Promise<RawSeedData> {
  const raw = await readFile(filePath, "utf8");
  return JSON.parse(raw);
}

async function loadSeedData(): Promise<ParsedSeedData> {
  const parsed: RawSeedData = await readJsonFile(DEFAULT_DATA_FILE);
...
function isLiveMatch(match: MatchData): boolean {
...
function buildMatchTimes(seedMatch: SeedMatch): MatchTimes {
  const now = new Date();
...
async function createMatch(seedMatch: SeedMatch): Promise<MatchData> {
  ...
  const responsePayload = (await response.json()) as MatchResponse;
...
async function insertCommentary(
...
): Promise<CommentaryInsertResult> {
  ...
  const responsePayload = (await response.json()) as CommentaryResponse;
  return responsePayload.data;
}

function inningsRank(period: CricketPeriod | undefined): number {
  if (!period) {
    return 0;
  }
...
function normalizeCricketFeed(
  ...
  const sorted = [...entries].sort((a, b) => {
    ...
    const seqA: number = Number.isFinite(a.sequence)
      ? a.sequence!
      : Number.MAX_SAFE_INTEGER;
    const seqB: number = Number.isFinite(b.sequence)
      ? b.sequence!
      : Number.MAX_SAFE_INTEGER;
    if (seqA !== seqB) {
      return seqA - seqB;
    }
    const minA: number = Number.isFinite(a.minute)
      ? a.minute!
      : Number.MAX_SAFE_INTEGER;
    const minB: number = Number.isFinite(b.minute)
      ? b.minute!
      : Number.MAX_SAFE_INTEGER;
    return minA - minB;
  });
...
function expandFeedForMatches(
  ...
  for (const match of seedMatches) {
    ...
    if (
      !templateBySport.has(match.sport) &&
      match.id !== undefined &&
      byMatchId.has(match.id)
    ) {
  ...
  for (const match of seedMatches) {
    ...
    const existingMatch = matchById.get(match.id!);
    if (existingMatch) {
      expanded.push(
        ...cloneCommentaryEntries(
          templateEntries,
          { ...existingMatch, id: match.id! } as MatchData,
          { ...match, id: match.id! } as MatchData,
        ),
      );
    }
  }
...
function buildRandomizedFeed(
  feed: CommentaryEntry[],
  matchMap: Map<number, MatchEntry>,
): CommentaryEntry[] {
  const buckets = new Map<number | null, CommentaryEntry[]>();
  for (const entry of feed) {
    const key: number | null = Number.isInteger(entry.matchId)
      ? (entry.matchId as number)
      : null;
    if (!buckets.has(key)) {
      buckets.set(key, []);
    }
  ...
  for (const [matchId, entries] of Array.from(buckets)) {
    if (!Number.isInteger(matchId)) {
      continue;
    }
    const target = matchMap.get(matchId as number);
  ...
  const matchIds = Array.from(buckets.keys()).filter(
    (id) => id !== null,
  ) as number[];
...
async function seed(): Promise<void> {
  ...
  const matchesList: MatchData[] = await fetchMatches();

  const matchMap = new Map<number, MatchEntry>();
  const matchKeyMap = new Map<string, MatchData>();
